...
class SpelunkyDataset(Dataset):    
    def __init__(self, levels_dir, metrics_file, tile_to_idx):
        ...
        print(f"📁 Loaded {len(self.level_files)} valid levels")
...
def train_gan(
    ...
    lr_g=0.0002,
    lr_d=0.0001,
    style_loss_weight=0.01,  # 👈 Lowered from 0.1
    ...
    print(f"✨ Improvements: Gradient clipping + Label smoothing + Lower style weight")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (real_levels, style_vectors, _) in enumerate(pbar):
            ...
            # 👇 Clip generator gradients (CRITICAL!)
        ...
        print(f"\n[Epoch {epoch+1}/{num_epochs}] "
              ...
              f"G_adv: {history['g_loss_adv'][-1]:.4f} | "
              f"G_style: {history['g_loss_style'][-1]:.4f}")
...
def main():
    ...
    parser.add_argument('--style_weight', type=float, default=0.01)  # 👈 Default 0.01
